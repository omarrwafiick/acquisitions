...
export const listPendingApprovalsController = async (req, res, next) => {
  try {
    const data = await listPendingApprovalsService(req.body.options, {
    ...
    responseHandler(
      req,
      res,
      { message: 'pending approvals was found!', data },
      200
    );
  } catch (error) {
...
export const approveRequestController = async (req, res, next) => {
  try {
    ...
    responseHandler(
      req,
      res,
      { message: 'request status was changed to approved status!', data },
      200
    );
  } catch (error) {
    responseHandler(req, res, error, error.status || 400);
...
export const rejectRequestController = async (req, res, next) => {
  try {
    ...
    responseHandler(
      req,
      res,
      { message: 'request status was changed to reject status!', data },
      200
    );
  } catch (error) {
    responseHandler(req, res, error, error.status || 400);
