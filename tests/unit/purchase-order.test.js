...
const { sendEmailService, vendorEmailBodyBuilder } =
...
} = await import('#src/services/purchaseOrder.service.js');
...
describe('Purchase Order Service', () => {
  ...
  it('should reject non approved request', async () => {
    isUserLinkedToOrganizationService.mockResolvedValue({
      id: 7,
    });
    ...
    ).rejects.toThrow('Request was not approved or found');
  });

  it('should reject duplicate purchase order', async () => {
    isUserLinkedToOrganizationService.mockResolvedValue({
      id: 7,
    });
    ...
    ).rejects.toThrow('Purchase order already exists');
  });

  it('should require reason for lower amount', async () => {
    isUserLinkedToOrganizationService.mockResolvedValue({
      id: 7,
    });
  ...
  it('should send purchase order', async () => {
    await sharedStateTest(
      CONSTANTS.PURCHASE_ORDER.STATUS.AWAITING,

      CONSTANTS.PURCHASE_ORDER.STATUS.SENT
    );
  });

  it('should complete purchase order', async () => {
    await sharedStateTest(
      CONSTANTS.PURCHASE_ORDER.STATUS.SENT,

      CONSTANTS.PURCHASE_ORDER.STATUS.COMPLETED
    );
  });
});

const sharedStateTest = async (oldStatus, expectedStatus) => {
  isUserLinkedToOrganizationService.mockResolvedValue({
    id: 7,
  });
  ...
  vendorEmailBodyBuilder.mockReturnValue('email body');

  sendEmailService.mockResolvedValue(true);
